...
export class AssetResolver {
  ...
  async assets(
    ...
    const filteredAssets = ASSETS.filter(
      ...
      .filter(({ id: originalId }) => id === -1 || originalId === id);

    const startCursor = (page - 1) * pageSize;
    const endCursor = Math.min(filteredAssets.length, page * pageSize);

    return {
      assets: (filteredAssets as any).slice(startCursor, endCursor),
      hasMore: !!filteredAssets.length && endCursor < filteredAssets.length,
    };
  }
